...
class MLFlowLogger(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        self.mlflow.log_metric('train acc', logs['accuracy'], step=epoch)
        self.mlflow.log_metric('val acc', logs['val_accuracy'], step=epoch)
        self.mlflow.log_metric('train loss', logs['loss'], step=epoch)
        self.mlflow.log_metric('val_loss', logs['val_loss'], step=epoch)
        self.mlflow.log_metric('lr', logs['lr'], step=epoch)
...
class MLFlowHandler:
    def __init__(self, model_name, run_name, mlflow_source='./mlruns', run_ngrok=True, ngrok_token=''):
        self.mlflow = mlflow
    ...
    @staticmethod
    def colab_ngrok(mlflow_source, ngrok_token):
    ...
    def start_run(self, args):
        ...
        if self.run_ngrok:
            self.colab_ngrok(self.mlflow_source, self.ngrok_token)
